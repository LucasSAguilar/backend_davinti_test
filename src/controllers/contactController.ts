...
import { deleteContactService, getAllContacts, getOneContact, isContact, sendNewContact, updateContactService } from "../services/contactService.js";
...
export const getContact = async (req: Request, res: Response) => {
    ...
    try {
        ...
        contact ? res.json(contact) : res.status(404).json({ message: 'Contato não encontrado' })
    } catch (error) {
        res.status(500).json({ message: error })
    }
}
